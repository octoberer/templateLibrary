...
type allTaskKey =
  | basicTaskId
  | processControlTaskId
  | templateTaskInstanceKey;
...
export interface basicTaskInstanceDefine {
  id: string;
  instanceId: string;
  inputTaskKeys: allTaskKey[];
  outputTaskKeys: allTaskKey[];
  parId: templateId;
  gen?: boolean;
}
//============模板相关
...
export interface templateTaskInstanceDefine {
  id: string; //当前解析器模板的唯一id
  instanceId: string; //当前解析器固定的输入输出task的唯一id-+
  taskInstanceId: string; //固定parId的唯一id
  parId: templateId;
  gen?: boolean;
  memoChildren: string[];
}
