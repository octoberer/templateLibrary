...
export default function FinishedTaskUI({ properties }: { properties: { [key: string]: any } }) {
    const { inputArgs, outputArgs, outputhandlArg, inputhandlArg, handle } = properties;
    const inputArgLabels: any[] = useMemo(
        ()=>inputArgs.map((item) => item.label),
        [inputArgs]
    );
    const outputArgLabels: any[] = useMemo(
        ()=>outputArgs.map((item) => item.label),
        [outputArgs]
    );
    ...
    return (
        <div className={styles.wrapper}>
            <Descriptions title="任务细节" bordered column={1}>
                ...
                <Descriptions.Item label="操作方式">{`${outputhandlArg.label}=${inputhandlArg[0].label} ${convertToSymbol(handle)} ${
